...
/*
Plugin Name: Contact Form 7 - Gateway IranDargah v1.0
Description: اتصال فرم های Contact Form 7 به درگاه پرداخت IranDargah
Version: 1.1
 */

function IRD_CF7_relative_time($ptime)
{
...
function result_payment_func($atts)
{
    global $wpdb;
    $Return_MessageEmail = '';
    if (!array_key_exists('authority', $_POST) && !array_key_exists('code', $_POST)) {
        return;
    }

    $Return_Trans_id = $_POST['authority'];
    $code = $_POST['code'];
    ...
    if ($Return_MessageEmail == 'success') {
        $wpdb->update($wpdb->prefix . 'cfIRD7_transaction', array('status' => 'success', 'transid' => $result->refId), array('transid' => $Return_Trans_id), array('%s', '%s'), array('%d'));
        ...
        $wpdb->update($wpdb->prefix . 'cfIRD7_transaction', array('status' => 'error'), array('transid' => $Return_Trans_id), array('%s'), array('%d'));
        //Dispaly
...
function IRD_Contant_Form_7_Gateway_install()
{

}

$dir = plugin_dir_path(__FILE__);
//register_activation_hook($dir . 'gateway_func.php', 'IRD_Contant_Form_7_Gateway_install');
...
if (is_plugin_active('contact-form-7/wp-contact-form-7.php')) {
    ...
    function cf7pp_admin_menu()
    {
        $addnew = add_submenu_page('wpcf7',
            __('تنظیمات ایران درگاه', 'contact-form-7'),
            __('تنظیمات ایران درگاه', 'contact-form-7'),
    ...
    function cf7pp_admin_list_trans()
    {
        ...
        if (count($transactions) == 0) {
            ...
            foreach ($transactions as $transaction) {
                ...
                echo '<br />(';
                echo IRD_CF7_relative_time($transaction["created_at"]);
                echo ' قبل)</td>';
    ...
    function cf7pp_admin_table()
    {
        ...
        if (isset($_POST['update'])) {
            ...
            if (!empty($_POST['irdargahgate'])) {
                $options['irdargahgate'] = 1;
            } else {
                $options['irdargahgate'] = 0;
            }
...
<br/><br/><br/>
 ...
 green
		  ...
		  <td>استفاده از درگاه پرداخت ایران گیت    </td>
            <td>
            <input type="checkbox" name="irdargahgate" value="1" ' . $checked . '>
			<span style="color:red">برای استفاده از درگاه ایران گیت باید ابتدا ان را بر روی درگاه خود فعال کنید</span>
            </td>
